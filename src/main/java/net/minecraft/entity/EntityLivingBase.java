...
import dev.Fall.event.impl.player.JumpFixEvent;
import dev.Fall.event.impl.player.LivingDeathEvent;
...
public abstract class EntityLivingBase extends Entity {
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (entity != null) {
            if (entity instanceof EntityLivingBase) {
                Fall.INSTANCE.getEventProtocol().handleEvent(new LivingDeathEvent(this, cause));
            }
    ...
    private int getArmSwingAnimationEnd() {
        return (Fall.INSTANCE.getModuleCollection().get(Animations.class).isEnabled() ? Animations.slowdown.getValue().intValue() : 1)
    ...
    protected void jump() {
        ...
        if (this.isSprinting()) {
            ...
            if (this instanceof EntityPlayerSP) {
                Fall.INSTANCE.getEventProtocol().handleEvent(jumpFixEvent);
    ...
    public void onLivingUpdate() {
        ...
        Double i = PacketFix.fixMotion();

        if (Math.abs(this.motionX) < i) {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < i) {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < i) {
            this.motionZ = 0.0D;
        }
