...
import dev.Fall.event.impl.player.StepConfirmEvent;
...
public abstract class Entity implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            Fall.INSTANCE.getEventProtocol().handleEvent(safeWalkEvent);
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    Fall.INSTANCE.getEventProtocol().handleEvent(new StepConfirmEvent());
                }
            }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if(player) {
            PlayerMoveUpdateEvent event = new PlayerMoveUpdateEvent(strafe, forward, friction, rotationYaw);
    ...
    public net.minecraft.util.Vec3d getLook(float partialTicks, float yaw, float pitch) {
        return this.getVectorForRotation_Vec3D(pitch, yaw);
    }
    ...
    public final net.minecraft.util.Vec3d getVectorForRotation_Vec3D(float pitch, float yaw) {
    ...
    public net.minecraft.util.Vec3d getPositionEyes_Vec3D(float partialTicks) {
        if (partialTicks == 1.0F) {
            return new net.minecraft.util.Vec3d(this.posX, this.posY + (double) this.getEyeHeight(), this.posZ);
        } else {
            double d0 = this.prevPosX + (this.posX - this.prevPosX) * (double) partialTicks;
            double d1 = this.prevPosY + (this.posY - this.prevPosY) * (double) partialTicks + (double) this.getEyeHeight();
            double d2 = this.prevPosZ + (this.posZ - this.prevPosZ) * (double) partialTicks;
            return new net.minecraft.util.Vec3d(d0, d1, d2);
        }
    }
