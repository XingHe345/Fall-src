...
import dev.Fall.vialoadingbase.netty.event.CompressionReorderEvent;
    ...
    public static NetworkManager createNetworkManagerAndConnect(InetAddress address, int serverPort, boolean useNativeTransport) {
        ...
        ((Bootstrap)((Bootstrap)((Bootstrap)(new Bootstrap()).group((EventLoopGroup)lazyloadbase.getValue())).handler(new ChannelInitializer<Channel>() {
            protected void initChannel(Channel p_initChannel_1_) throws Exception {
                ...
                if (p_initChannel_1_ instanceof SocketChannel && dev.Fall.vialoadingbase.ViaLoadingBase.getInstance().getTargetVersion().getVersion() != 47) {
                    UserConnection user = new UserConnectionImpl(p_initChannel_1_, true);
                    new ProtocolPipelineImpl(user);
                    p_initChannel_1_.pipeline().addLast(new ChannelHandler[]{new dev.Fall.viamcp.MCPVLBPipeline(user)});
                }
