...
import dev.Fall.event.impl.render.CustomBlockRenderEvent;
import dev.Fall.module.impl.combat.KillAura;
import dev.Fall.module.impl.render.Animations;
...
public class ItemRenderer {
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress) {
        boolean animations = Fall.INSTANCE.isEnabled(Animations.class);
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            if (Fall.INSTANCE.isEnabled(Animations.class)) {
                                switch (Animations.mode.getMode()) {
                                    ...
                                    case "Custom":
                                        ...
                                        Fall.INSTANCE.getEventProtocol().handleEvent(blockRenderEvent);
                                        break;
                                    case "Wood":
                                        this.transformFirstPersonItem(f / 2.0F, 0.0f);
                                        GlStateManager.translate(0.0F, 0.3F, -0.0F);
                                        GlStateManager.rotate(-var15 * 30.0F, 1.0F, 0.0F, 2.0F);
                                        GlStateManager.rotate(-var15 * 44.0F, 1.5F, (var15 / 1.2F), 0.0F);
                                        this.doBlockTransformations();
                                        break;
                                    case "Shove":
                                        this.transformFirstPersonItem(f / 2.0F, swingProgress);
                                        GlStateManager.translate(0.03, -0.055, 0.05);
                                        GlStateManager.rotate(-nekker * 42.0f, nekker / 2.0f, 0.0f, 9.0f);
                                        GlStateManager.rotate(-nekker * 50.0f, 0.8f, nekker / 2.0f, 0.0f);;
                                        this.doBlockTransformations();
                                        break;
                                }
                            } else {
