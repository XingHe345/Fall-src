...
import dev.Fall.module.impl.render.ESP2D;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    protected void renderModel(T entitylivingbaseIn, float x, float y, float z, float entityYaw, float entityPitch, float scaleFactor, float partialTicks) {
        ...
        if (flag || flag1) {
            ...
            Fall.INSTANCE.getEventProtocol().handleEvent(renderModelEvent);
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (entity instanceof EntityPlayer) {
            ...
            Fall.INSTANCE.getEventProtocol().handleEvent(nametagRenderEvent);
            if (nametagRenderEvent.isCancelled()) return;
        }
        if (esp2D == null) {
            esp2D = Fall.INSTANCE.getModuleCollection().getModule(ESP2D.class);
        }
