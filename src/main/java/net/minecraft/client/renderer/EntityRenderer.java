...
import dev.Fall.module.impl.render.Ambience;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            for (Entity entity1 : list) {
                float f1 = entity1.getCollisionBorderSize() + ((entity instanceof EntityPlayer && !entity.isInvisible() ? PacketFix.fixHitbox() : 0));
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
            ...
            Fall.INSTANCE.getEventProtocol().handleEvent(hurtCamEvent);
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            float f = this.mc.gameSettings.mouseSensitivity * 0.6F + 0.2F;
            float f1 = f * f * f * (ChestStealer.canSteal() ? 10.0F : 8.0F);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        Fall.INSTANCE.getEventProtocol().handleEvent(new Render3DEvent(partialTicks));
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    ...
                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
                        ...
                        if (k2 != l2) {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F && !Ambience.overrideSnow) {
                                if (j1 != 0) {
                                    if (j1 >= 0) {
                                        tessellator.draw();
                                    }
