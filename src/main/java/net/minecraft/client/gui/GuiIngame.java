...
public class GuiIngame extends Gui implements Utils {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Fall.INSTANCE.getEventProtocol().handleEvent(new PreRenderEvent());


        PostProcessing postProcessing = (PostProcessing) Fall.INSTANCE.getModuleCollection().get(PostProcessing.class);
        postProcessing.blurScreen();


        Fall.INSTANCE.getEventProtocol().handleEvent(new Render2DEvent(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight()));
        NotificationsMod notif = Fall.INSTANCE.getModuleCollection().getModule(NotificationsMod.class);
        if (notif.isEnabled()) {
            notif.render();
        }
        ...
        if (scoreobjective1 != null && Fall.INSTANCE.isEnabled(ScoreboardMod.class)) {
