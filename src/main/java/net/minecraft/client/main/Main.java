...
public class Main {
    ...
    public static void main(String[] p_main_0_) {
        ...
        OptionSpec<String> optionspec9 = optionparser.accepts("username").withRequiredArg().defaultsTo("Fluid" + Minecraft.getSystemTime() % 1000L);
        ...
        OptionSpec<String> intentApiKey = optionparser.accepts("intentApiKey").withOptionalArg().ofType(String.class).defaultsTo("");
        OptionSpec<String> optionspec19 = optionparser.nonOptions();
        OptionSet optionset = optionparser.parse(p_main_0_);
        List<String> list = optionset.valuesOf(optionspec19);

        if (optionset.valueOf(intentApiKey) != null && !optionset.valueOf(intentApiKey).isEmpty()) {
            apiKey = optionset.valueOf(intentApiKey);
        } else {
            String key = System.getProperty("intentApiKey");
