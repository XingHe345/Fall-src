...
import dev.Fall.utils.player.BlockUtils;
import dev.Fall.event.impl.player.BoundingBoxEvent;
...
public class Block
{
        ...
        if (XRay.enabled && XRay.isWhitelisted(BlockUtils.getBlockAtPos(pos))) {
            if (XRay.bypass.isEnabled()) {
                ...
                for (EnumFacing si : EnumFacing.VALUES) {
                    if (XRay.isExposed(worldIn, pos.offset(si), si, minY, maxY, minZ, maxZ, minX, maxX)) {
                        return true;
                    }
                }
            } else {
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
        ...
        if(collidingEntity == Minecraft.getMinecraft().thePlayer) {
            ...
            Fall.INSTANCE.getEventProtocol().handleEvent(boundingBoxEvent);
