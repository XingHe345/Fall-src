...
import dev.Fall.ui.altmanager.helpers.Alt;
import dev.Fall.ui.altmanager.helpers.AltManagerUtils;
...
import dev.Fall.utils.objects.DoubleIconButton;
import dev.Fall.utils.objects.Scroll;
import dev.Fall.utils.objects.TextField;
import dev.Fall.utils.render.*;
import dev.Fall.utils.server.ban.HypixelBan;
...
public class AltPanel extends Panel {
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (Fall.INSTANCE.getAltManager().isTyping()) return;
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        TextField searchField = Fall.INSTANCE.getAltManager().searchField;
        boolean filterBanned = Fall.INSTANCE.getAltManager().filterBanned.isEnabled();
    ...
    public static class AltRect implements Screen {
        ...
        public void drawScreen(int mouseX, int mouseY) {
            ...
            if (alt.username != null) {
                usernameText = FluidBoldFont16.trimStringToWidth(alt.username, (int) (width - (altSize + 10)));
            }
            float totalHeight = FluidBoldFont16.getHeight() + 6 + FluidFont14.getHeight();
            ...
            if (alt.hypixelBan != null) {
                ...
                if (diff > 0 || ban.getUnbanDate() == 0) {
                    hasBan = true;
                    totalHeight += FluidBoldFont14.getHeight() + 4;
                }
            }
            ...
            FluidBoldFont16.drawString(usernameText, x + 3 + altSize + 3.5f, usernameY, textColor);

            float typeX = x + 7 + altSize + FluidBoldFont16.getStringWidth(usernameText);
            float size = FluidBoldFont16.getHeight() + 2;
            switch (alt.getType()) {
                case "Microsoft":
                ...
                case "Mojang":
                    ...
                    mc.getTextureManager().bindTexture(new ResourceLocation("Fluid/mojang.png"));
                    Gui.drawModalRectWithCustomSizedTexture(typeX + 2, usernameY - 1, 0, 0, size, size, size, size);
                    RenderUtil.resetColor();
                    break;
                case "Cracked":
                    Color red = Fall.INSTANCE.getSideGui().getRedBadColor();
                    FluidBoldFont14.drawString(" Cracked", typeX, usernameY + 1, ColorUtil.applyOpacity(red, 1 - credsAnim));
                    break;
                case "Not logged in":
                    FluidBoldFont14.drawString("§bNot logged in", typeX, usernameY + 1, ColorUtil.applyOpacity(-1, 1 - credsAnim));
                    break;
            }

            drawBan(x + altSize + 6.5f, usernameY + FluidBoldFont16.getHeight() + 6, 1 - credsAnim);
            if (hasBan) {
                usernameY += FluidBoldFont14.getHeight() + 6;
            }
            ...
            hoveringCreds = HoveringUtil.isHovering(x + 3 + altSize + 1.5f, (usernameY + FluidBoldFont16.getHeight() + 6) - 2,
                    FluidFont14.getStringWidth("§n" + credsText) + 4, FluidFont14.getHeight() + 4, mouseX, mouseY);

            float opacity = showCreds ? (hoveringCreds ? .75f : 0.5f) : hoveringCreds ? .5f : .35f;
            FluidFont14.drawString("§n" + credsText, x + 3 + altSize + 3.5f,
                    usernameY + FluidBoldFont16.getHeight() + 6, ColorUtil.applyOpacity(-1, opacity));
            ...
            if (showCreds || !credsAnimation.isDone()) {
                ...
                if (alt.email != null) {
                    FluidFont14.drawString("Email: " + alt.email, xVal, yVal, ColorUtil.applyOpacity(-1, credsAnim));
                    yVal += FluidFont14.getHeight() + 4;
                }

                FluidFont14.drawString("Password: " + alt.password, xVal, yVal, ColorUtil.applyOpacity(-1, credsAnim));
                RenderUtil.scaleEnd();
            }


            if (removeShit) return;
            if (Fall.INSTANCE.getAltManager().currentSessionAlt == alt) {
                Color green = Fall.INSTANCE.getSideGui().getGreenEnabledColor();
                FluidBoldFont14.drawString("Logged in", x + width - (FluidBoldFont14.getStringWidth("Logged in") + 5),
                        y + height - (FluidBoldFont14.getHeight() + 5), green);
            } else {
        ...
        public void drawAltHead(float x, float y, float size) {
            Fall.INSTANCE.getAltManager().getUtils().getHead(alt);
            GLUtil.startBlend();
            RenderUtil.setAlphaLimit(0);
            mc.getTextureManager().bindTexture(alt.head != null ? alt.head : new ResourceLocation("Fluid/X-Steve.png"));
        ...
        public void drawBan(float x, float y, float alpha) {
            if (alt.hypixelBan != null) {
                ...
                if (diff > 0) {
                    ...
                    FluidFont14.drawString("§lBanned §ron Hypixel for §l" + str, x, y, ColorUtil.applyOpacity(-1, alpha * .5f));
                } else if (ban.getUnbanDate() == 0) {
                    FluidFont14.drawString("§lBanned §ron Hypixel permanently", x, y, ColorUtil.applyOpacity(-1, alpha * .5f));
                }
            }
        }
    }
