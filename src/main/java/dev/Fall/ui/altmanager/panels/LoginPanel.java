...
import dev.Fall.utils.text.RandomStringGenerator;
...
public class LoginPanel extends Panel {
    ...
    public LoginPanel() {
        ...
        textFields.add(new TextField(FluidFont20));
        textFields.add(new TextField(FluidFont20));
    }
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        setHeight(180);
        FluidBoldFont32.drawCenteredString("Login", getX() + getWidth() / 2f, getY() + 3, ColorUtil.applyOpacity(-1, .75f));
        ...
        for (ActionButton actionButton : actionButtons) {
            ...
            actionButton.setFont(FluidBoldFont22);

            actionButton.setClickAction(() -> {
                switch (actionButton.getName()) {
                    case "Login":
                        Fall.INSTANCE.getAltManager().getUtils().login(textFields.get(0), textFields.get(1));
                        resetTextFields();
                        Fall.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                    case "Add":
                        TextField username = textFields.get(0);
                        ...
                        break;
                    case "Gen Cracked":
                        Fall.INSTANCE.getAltManager().getUtils().loginWithString(generator.generate(8), "", false);
                        Fall.INSTANCE.getAltManager().getAltPanel().refreshAlts();
                        break;
                }
            });
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("Fluid/mc.png"));
        ...
        FluidBoldFont26.drawString("Microsoft Login", microX + 10, microsoftY + 4, -1);

        FluidFont16.drawString("Login to your migrated account", microX + 10, microsoftY + 23, -1);
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (hoveringMicrosoft && button == 0) {
            ...
            Fall.INSTANCE.getAltManager().getUtils().microsoftLoginAsync(email, password);
            resetTextFields();
        }
