...
import dev.Fall.ui.sidegui.utils.ToggleButton;
...
public class ConfigPanel extends Panel {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (voteRefreshTimer.hasTimeElapsed(30000)) {
            Multithreading.runAsync(() -> Fall.INSTANCE.getCloudDataManager().refreshVotes());
            voteRefreshTimer.reset();
        }


        Fall.INSTANCE.getCloudDataManager().applyVotes();

        FluidBoldFont40.drawString("Configs", getX() + 8, getY() + 8, getTextColor());
            ...
            button.setClickAction(() -> {
                switch (button.getName()) {
                    case "Upload config":
                        ...
                        uploadForm.setTriUploadAction((name, description, server) -> {
                            Multithreading.runAsync(() -> {
                                String fileData = Fall.INSTANCE.getConfigManager().serialize();
                    ...
                    case "Save current config":
                        Form saveForm = Fall.INSTANCE.getSideGui().displayForm("Save Config");
                        saveForm.setUploadAction((name, description) -> {
                            Multithreading.runAsync(() -> {
                                Fall.INSTANCE.getConfigManager().saveConfig(name);
                                Fall.INSTANCE.getCloudDataManager().refreshData();
                            });
                        });
                        break;
        ...
        switch (carouselButtons.getCurrentButton()) {
            case "Cloud":
                ...
                if (!sortingSelection.equals(sorting.getSelection())) {
                    Multithreading.runAsync(() -> Fall.INSTANCE.getCloudDataManager().refreshData());
                    sortingSelection = sorting.getSelection();
                }

                if (Fall.INSTANCE.getCloudDataManager().isRefreshing()) return;
                ...
                for (CloudConfigRect cloudConfigRect : cloudConfigRects) {
                    ...
                    if (filterAuthor) {
                        ...
                        if (!Fall.INSTANCE.getIntentAccount().username.equals(configAuthor)) continue;
                    }

                    if (filterVersion && !cloudConfigRect.getConfig().getVersion().equals(Fall.VERSION)) continue;
...
//        ConfigManager.loadVisuals = toggleButtons.get(0).isEnabled();
        ConfigManager.loadVisuals = true;
    ...
    public void refresh() {
        ...
        for (CloudConfig cloudConfig : Fall.INSTANCE.getCloudDataManager().getCloudConfigs()) {
    ...
    public Comparator<CloudConfigRect> relevanceSorting() {
        return Comparator.<CloudConfigRect>comparingInt(configRect -> {
            ...
            versionScore = cloudConfig.getVersion().equals(Fall.VERSION) ? 100 : 0;
            ...
            int score = (serverScore * serverSameWeight) + (recentlyUploadedScore * recentlyUploadedWeight) + (versionScore * versionSameWeight) + (voteRatioScore * voteRatioWeight);
