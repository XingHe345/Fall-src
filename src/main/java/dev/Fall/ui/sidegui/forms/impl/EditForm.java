...
public class EditForm extends Form {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        float infoX = getX() + FluidBoldFont40.getStringWidth(getTitle()) + 15;
        float infoY = getY() + 3 + FluidFont16.getMiddleOfBox(FluidBoldFont40.getHeight()) + 2;

        FluidFont16.drawString("Choose to either update or delete your " + type.toLowerCase(),
                infoX, infoY, ColorUtil.applyOpacity(getTextColor(), .5f));

        FluidBoldFont16.drawString(data.getName(), getX() + 5, getY() + 5.5f + FluidBoldFont40.getHeight(), getAccentColor());
        ...
        FluidBoldFont26.drawString("Update", updateX + 5, updateY + 3, getTextColor());
        ...
        descriptionField.setFont(FluidFont16);
        ...
        FluidFont18.drawString("Description", descriptionField.getXPosition(),
                descriptionField.getYPosition() - (FluidFont18.getHeight() + 5), getTextColor());
        ...
        if (error) {
            FluidFont16.drawCenteredStringWithShadow("Error please fill out the required fields",
                    updateButton.getX() + updateButton.getWidth() / 2f, updateButton.getY() - (FluidFont16.getHeight() + 5),
                    Fall.INSTANCE.getSideGui().getRedBadColor().getRGB());
        }

        if (script) {
            FluidFont16.drawString("Update the script with one of your local scripts",
                    updateX + 10 + FluidBoldFont26.getStringWidth("Update"),
                    updateY + 4 + FluidFont16.getMiddleOfBox(FluidBoldFont26.getHeight()), ColorUtil.applyOpacity(getTextColor(), .5f));
            ...
            FluidFont16.drawString("Update the config with your current settings",
                    updateX + 10 + FluidBoldFont26.getStringWidth("Update"),
                    updateY + 4 + FluidFont16.getMiddleOfBox(FluidBoldFont26.getHeight()), ColorUtil.applyOpacity(getTextColor(), .5f));
        }
        ...
        FluidBoldFont26.drawString("Delete", deleteX + 5, deleteY + 3, getTextColor());
        iconFont26.drawString(FontUtil.TRASH, deleteX + 10 + FluidBoldFont26.getStringWidth("Delete"), deleteY + 4.5f, getTextColor());

        FluidFont16.drawString("Delete the " + type.toLowerCase() + " from the", deleteX + 5, deleteY + 22, ColorUtil.applyOpacity(getTextColor(), .5f));
        FluidFont16.drawString("cloud", deleteX + 5, deleteY + 22 + FluidFont16.getHeight() + 3, ColorUtil.applyOpacity(getTextColor(), .5f));
        ...
        deleteButton.setColor(Fall.INSTANCE.getSideGui().getRedBadColor());
    ...
    public void setup(CloudData data, boolean script) {
        ...
        if (script) {
            scriptFiles.setupOptions(Fall.INSTANCE.getScriptManager().getScriptFileNameList());
        }
