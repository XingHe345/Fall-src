      ...
      if (stack.stackSize == 0) {
         ...
         if (worldIn.setBlockState(pos, iblockstate1, 3)) {
            ...
            if (ViaLoadingBase.getInstance().getTargetVersion().getOriginalVersion() != 47) {
               mc.theWorld.playSoundAtPos(pos.add(0.5D, 0.5D, 0.5D), iblock.getBlock().stepSound.getPlaceSound(), (iblock.getBlock().stepSound.getVolume() + 1.0F) / 2.0F, iblock.getBlock().stepSound.getFrequency() * 0.8F, false);
            } else {
