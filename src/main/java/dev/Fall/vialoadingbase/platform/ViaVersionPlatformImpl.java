...
import dev.Fall.vialoadingbase.platform.viaversion.VLBViaAPIWrapper;
import dev.Fall.vialoadingbase.platform.viaversion.VLBViaConfig;
import dev.Fall.vialoadingbase.util.VLBTask;
import java.io.File;
...
public class ViaVersionPlatformImpl implements ViaPlatform<UUID> {
   ...
   public static List<ProtocolVersion> createVersionList() {
      List<ProtocolVersion> versions = (List)(new ArrayList(ProtocolVersion.getProtocols())).stream().filter((protocolVersion) -> {
   ...
   public String getPluginVersion() {
      return "4.7.0-1.20-pre2-SNAPSHOT";
   }
