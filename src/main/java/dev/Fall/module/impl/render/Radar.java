...
public class Radar extends Module {
    ...
    public final Dragging drag = Fall.INSTANCE.createDrag(this, "radar", 5, 40);
    ...
    public void onShaderEvent(ShaderEvent e) {
        ...
        if (e.getBloomOptions().getSetting("Radar").isEnabled()) {
            RoundedUtil.drawGradientRound(x, y, size, size, 0, colorWheel.getColor1(), colorWheel.getColor4(), colorWheel.getColor2(), colorWheel.getColor3());
        } else {
            RoundedUtil.drawRound(x, y, size, size, 0, Color.BLACK);
    ...
    public void onRender2DEvent(Render2DEvent e) {
        ...
        float alpha = .85f;
        RoundedUtil.drawRound(x, y, size, size, 0, new Color(0, 0, 0, 80));
        RoundedUtil.drawRound(x, y, size, 10, 0, new Color(0, 0, 0, 35));
        Fall.fluxICON16.drawString("b", x + 2, y + 2.5f, -1);
        Fall.FluidFont16.drawString("Radar", x + 4 + Fall.fluxICON16.getStringWidth("b"), y + 2, -1);

        Gui.drawRect2(x - 1, y + (size / 2f - .5), size + 2, 1, lineColor.getRGB());
        Gui.drawRect2(x + (size / 2f - .5), y + 11, 1, size + 2 - 12.5f, lineColor.getRGB());
