...
import dev.Fall.utils.spotify.SpotifyAPI;
...
public class SpotifyMod extends Module {
    ...
    private final Dragging drag = Fall.INSTANCE.createDrag(this, "spotify", 5, 150);
    ...
    public void onRender2DEvent(Render2DEvent event) {
        ...
        RenderUtil.scissor(x + albumCoverSize, y, playerWidth, height, () -> {
            ...
            boolean needsToScrollTrack = FluidBoldFont26.getStringWidth(currentTrack.getName()) > playerWidth;
            boolean needsToScrollArtist = FluidFont22.getStringWidth(artistsDisplay.toString()) > playerWidth;

            float trackX = (float) (((x + albumCoverSize) - FluidBoldFont22.getStringWidth(currentTrack.getName())) +
                    ((FluidBoldFont22.getStringWidth(currentTrack.getName()) + playerWidth) * scrollTrack.getLinearOutput()));

            FluidBoldFont22.drawString(currentTrack.getName(), needsToScrollTrack ? trackX : x + albumCoverSize + 3, y + 3, -1);

            float artistX = (float) (((x + albumCoverSize) - FluidFont18.getStringWidth(artistsDisplay.toString())) +
                    ((FluidFont18.getStringWidth(artistsDisplay.toString()) + playerWidth) * scrollArtist.getLinearOutput()));

            FluidFont18.drawString(artistsDisplay.toString(), needsToScrollArtist ? artistX : x + albumCoverSize + 4, y + 17, -1);
        });

        //Draw time left on song
        FluidFont16.drawString(trackRemaining, x + width - (FluidFont16.getStringWidth(trackRemaining) + 3),
                y + height - (FluidFont16.getHeight() + 3), -1);
