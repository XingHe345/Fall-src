...
public class ChestStealer extends Module {
    ...
    private final BooleanSetting sendC0f = new BooleanSetting("sendC0F", false);
    ...
    public ChestStealer() {
        ...
        this.addSettings(delay, aura, auraRange, stealingIndicator, titleCheck, freeLook, reverse, silent, smart, sendC0f);
    }


    @Override
    public void onPacketSendEvent(PacketSendEvent event) {
        if(sendC0f.isEnabled()) {
    ...
    public void onMotionEvent(MotionEvent e) {
        if (e.isPre()) {
            ...
            if (invManager == null) invManager = Fall.INSTANCE.getModuleCollection().getModule(InvManager.class);
            if (aura.isEnabled()) {
    ...
    public void onRender2DEvent(Render2DEvent event) {
        if (stealingIndicator.isEnabled() && stealing) {
            ...
            AbstractFontRenderer fr = WeiLaiYuan.size(16);
    ...
    public static boolean canSteal() {
        if (Fall.INSTANCE.isEnabled(ChestStealer.class) && mc.currentScreen instanceof GuiChest) {
            ContainerChest chest = (ContainerChest) mc.thePlayer.openContainer;
            IInventory chestInv = chest.getLowerChestInventory();
