...
public class RotationUtils implements Utils {
    ...
    public static Vector2f calculate(final Vector3d from, final Vector3d to) {
        ...
        final float yaw = (float) (MathHelper.atan2(diff.getZ(), diff.getX()) * (180.0F / PI)) - 90.0F;
        final float pitch = (float) (-(MathHelper.atan2(diff.getY(), distance) * (180.0F / PI)));
        return new Vector2f(yaw, pitch);
    }
    ...
    public static float[] targetRotation(){
        Module ka = Fall.INSTANCE.getModuleCollection().get(KillAura.class);
        if(!ka.isEnabled()) return null;

        return new float[]{KillAura.target.rotationYaw, KillAura.target.rotationPitch};
    }
