package dev.Fall.utils.font.hanabi.noway.ttfr;
 ...
 * </pre>
 */
@SuppressWarnings({"UnusedReturnValue", "UnnecessaryUnicodeEscape", "unused", "DuplicatedCode", "SameParameterValue"})
public class HStringCache {
    ...
    public int getStringHeight(String str) {
        ...
        for (int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++) {
            ...
            if (c >= '0' && c <= '9') {
                texture = digitGlyphs[Font.PLAIN][c - '0'].texture;
            }
    ...
    public int renderString(String str, float startX, float startY, int initialColor) {
        ...
        for (int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++) {
            ...
            while (colorIndex < entry.colors.length && entry.glyphs[glyphIndex].stringIndex >= entry.colors[colorIndex].stringIndex) {
                color = applyColorCode(entry.colors[colorIndex].colorCode, initialColor);
