    ...
    public void execute(String[] args) {
        if (args.length != 1) {
            ...
            Multithreading.runAsync(() -> {
                ...
                if (isScript) {
                    File scriptFile = new File(Minecraft.getMinecraft().mcDataDir + "/Fall/Scripts/" + name + ".js");

                    downloadScriptToFile(scriptFile, objectData);

                    Fall.INSTANCE.getSideGui().getScriptPanel().refresh();
                } else {
                    if (Fall.INSTANCE.getConfigManager().loadConfig(objectData, false)) {
