...
import dev.Fall.utils.misc.StringUtils;
...
public class CloudUtils {
    ...
    public static String postOnlineConfig(String name, String description, String server, String data) {
        JsonObject response = Cloud.begin(RequestType.UPLOAD)
                .modifyGetMap((map) -> {
                    map.put("name", name);
                    map.put("meta", "script:false:version:" + Fall.VERSION);
    ...
    public static String postOnlineScript(String name, String description, String data) {
        JsonObject response = Cloud.begin(RequestType.UPLOAD)
                .modifyGetMap((map) -> {
                    map.put("name", name);
                    map.put("meta", "script:true:version:" + Fall.VERSION);
                    //Char limit 240 for description
                    map.put("description", description);
                    map.put("published", "1");
                })
    ...
    public static boolean updateData(String shareCode, String description, String data, boolean script) {
        JsonObject response = Cloud.begin(RequestType.UPDATE)
                .modifyGetMap((map) -> {
                    ...
                    map.put("meta", "script:" + script + ":version:" + Fall.VERSION);
                    //Char limit 240 for description
                    map.put("description", description);
                })
                .modifyPostMap((map) -> map.put("body", data))
    ...
    public static boolean deleteData(String shareCode) {
        ...
        if (response != null) {
            if (response.get("response").isJsonObject()) {
                ...
                HttpsURLConnection connection = (HttpsURLConnection) new URL("https://senoe.win/Fluid/configs").openConnection();
                connection.setRequestProperty("User-Agent", "Fluid/" + Fall.VERSION);
                connection.setRequestProperty("Content-Type", "application/json");
                connection.setRequestProperty("Accept", "application/json");
                connection.setRequestMethod("POST");
                connection.setDoOutput(true);

                int uid = Fall.INSTANCE.getIntentAccount().client_uid;
    ...
    public static JsonObject getVoteData() {
        ...
        NetworkingUtils.HttpResponse r = NetworkingUtils.httpsConnection(String.format("https://senoe.win/Fluid/configs?uid=%s&i=%s&e=%s&u=%s&ak=%s",
    ...
    public static void vote(VoteType type, CloudData config) {
        try {
            ...
            if (r.getResponse() == 200) {
                ...
                Fall.INSTANCE.getCloudDataManager().refreshVotes();
